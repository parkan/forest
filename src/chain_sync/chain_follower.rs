    ...
    async fn execute(
        ...
        match self {
            ...
            SyncTask::ValidateTipset {
                ...
                // A parent state mismatch can be caused by locally corrupted computation
                // inputs rather than a bad block — e.g. a stale epoch-to-tipset lookup
                // entry feeding wrong randomness into state computation (a reorg deeper
                // than the finality calculator's estimate can leave one behind). Verify
                // and repair the lookup table over the randomness-reachable window; if
                // anything was actually wrong, drop the poisoned cached state and retry
                // the tipset instead of marking it bad. If the table was clean, the
                // mismatch is genuine and the tipset is marked bad as usual.
