...
pub enum TipsetSyncerError {
    ...
    Validation(String),
    /// The parent state or receipt root computed locally does not match the block header.
    /// Distinct from [`Self::Validation`] so the chain follower can attempt to repair
    /// locally corrupted inputs (e.g. a stale tipset lookup entry feeding wrong randomness
    /// into the computation) before treating the block as bad.
